...
Последовательностью в Python называется итерабельный объект (упорядоченная коллекция), который поддерживает эффективный доступ к элементам с использованием целочисленных индексов через специальный метод `__getitem__()` и поддерживает метод `__len__()`, который возвращает длину последовательности. Индексы с нуля до len() - 1,
...
### Какие операции поддерживают все последовательности
...
- `s[i:j]` – срез последовательности s от i до j (последний не входит)
- `s[i:j:k]` – срез последовательности s от i до j с шагом k
- `len(s)` – длина последовательности
- `min(s)` – минимальный элемент последовательности, можно добавить аргументом ключевую функцию min(seq, key=lambda x: ...(x))
- `max(s)` – максимальный элемент последовательности, можно добавить аргументом ключевую функцию max(seq, key=lambda x: ...(x))
- `sorted(s)` – сортировка,  можно добавить аргументом ключевую функцию и порядок сортировки  - sorted(seq, key=len, reverse=True), возвращает новый инстанс.
...
Неизвменяемые последовательнтсти  - str, tuple, bytes, range.Неизменяемые последовательности реализуют операцию hash(s) – хеш-значение объекта.

Изменяемые последовательности - list, bytearray. Большинство изменяемых последовательностей поддерживают следующие операции:

- `s[i] = x` – элемент с индексом i заменяется на x, дает IndexError, если индекса нет.
- `s.insert(i, x)` – вставка элемента x по индексу i, сли индекс за границами - добавит к концу.
- `s.append(x)` – добавление x в конец последовательности (реализуется как добавление ссылки в массив указателей, тип элемента не изменится)
- `s.extend(t)` – добавление всех элементов итерабельного объекта в конец последовательности в виде элементов этой последовательности (те в массив указателей добавятся указатели на эти элементы).
- `s[i:j] = t`, `s[i:j:k] = t` – элементы с индексами от i до j (с шагом k) заменяются содержимым итерабельного объекта t
- `del s[i:j]`, `del s[i:j:k]` – удаление соответствующих элементов из последовательности, возвращает тот же список, если индекса нет
- `del s[i]` – удаление элемента из последовательности по индексу, возвращает IndexError, если индекса нет.
- `s.clear()` – удаление всех элементов последовательности
- `s.remove(x)` – удаление первого вхождения x, возвращает IndexError, если индекса нет.
- `s.copy()` – нерекурсивная копия последовательности (то есть для составного объекта например, список списков на элементы дочернего списка будут сделанны ссылки, а не новые элементы)
- `s.pop()`, `s.pop(i)` – возврат значения по индексу i (по умолчанию – последний элемент) и удаление его из последовательности, индексы пересчитываются, возвращает IndexError, если индекса нет.
- `s.reverse()` – разворот последовательности в обратном порядке, выполняется в in-place (возвращает None)
